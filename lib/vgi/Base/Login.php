...
abstract class Login implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\vgi\\Map\\LoginTableMap';
    ...
    /**
     * Initializes internal state of vgi\Base\Login object.
     * @see applyDefaults()
     */
     ...
     * @return $this|\vgi\Login The current object (for fluent API support)
     */
    public function setLoginId($v)
    {
        ...
        if ($this->login_id !== $v) {
            $this->login_id = $v;
            $this->modifiedColumns[LoginTableMap::COL_LOGIN_ID] = true;
        }
    ...
    public function setRole($v)
    {
        ...
        if ($this->role !== $v) {
            $this->role = $v;
            $this->modifiedColumns[LoginTableMap::COL_ROLE] = true;
        }
    ...
    public function setUpdated($v)
    {
        ...
        if ($this->updated !== null || $dt !== null) {
            if ($this->updated === null || $dt === null || $dt->format("Y-m-d H:i:s.u") !== $this->updated->format("Y-m-d H:i:s.u")) {
                ...
                $this->modifiedColumns[LoginTableMap::COL_UPDATED] = true;
            }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : LoginTableMap::translateFieldName('LoginId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->login_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : LoginTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType)];
            $this->name = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : LoginTableMap::translateFieldName('Role', TableMap::TYPE_PHPNAME, $indexType)];
            $this->role = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : LoginTableMap::translateFieldName('Passwd', TableMap::TYPE_PHPNAME, $indexType)];
            $this->passwd = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : LoginTableMap::translateFieldName('Updated', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 5; // 5 = LoginTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\vgi\\Login'), 0, $e);
        }
    }
        ...
        $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                if ($this->archiveOnDelete) {
                    // do nothing yet. The object will be archived later when calling ChildLoginQuery::delete().
                } else {
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\vgi\Login The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(LoginTableMap::COL_LOGIN_ID)) {
            $criteria->add(LoginTableMap::COL_LOGIN_ID, $this->login_id);
        }
        if ($this->isColumnModified(LoginTableMap::COL_NAME)) {
            $criteria->add(LoginTableMap::COL_NAME, $this->name);
        }
        if ($this->isColumnModified(LoginTableMap::COL_ROLE)) {
            $criteria->add(LoginTableMap::COL_ROLE, $this->role);
        }
        if ($this->isColumnModified(LoginTableMap::COL_PASSWD)) {
            $criteria->add(LoginTableMap::COL_PASSWD, $this->passwd);
        }
        if ($this->isColumnModified(LoginTableMap::COL_UPDATED)) {
            $criteria->add(LoginTableMap::COL_UPDATED, $this->updated);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \vgi\Login (or compatible) type.
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($makeNew) {
            $copyObj->setNew(true);
            $copyObj->setLoginId(NULL); // this is a auto-increment column, so set to default value
        }
    }
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \vgi\Login Clone of current object.
     * @throws PropelException
     */
     ...
     *
     * @return     ChildLoginArchive An archive object, or null if the current object was never archived
     */
    public function getArchive(ConnectionInterface $con = null)
    {
        ...
        $archive = ChildLoginArchiveQuery::create()
            ->filterByPrimaryKey($this->getPrimaryKey())
            ->findOne($con);
     ...
     *
     * @return     ChildLoginArchive The archive object based on this object
     */
     ...
     *
     * @param      ChildLoginArchive $archive An archived object based on the same class
      * @param      Boolean $populateAutoIncrementPrimaryKeys
     *               If true, autoincrement columns are copied from the archive object.
     *               If false, autoincrement columns are left intact.
      *
     * @return     ChildLogin The current object (for fluent API support)
     */
    public function populateFromArchive($archive, $populateAutoIncrementPrimaryKeys = false) {
        if ($populateAutoIncrementPrimaryKeys) {
            $this->setLoginId($archive->getLoginId());
        }
        $this->setName($archive->getName());
        $this->setRole($archive->getRole());
        $this->setPasswd($archive->getPasswd());
        $this->setUpdated($archive->getUpdated());
