...
abstract class Style implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\vgi\\Map\\StyleTableMap';
    ...
    /**
     * Initializes internal state of vgi\Base\Style object.
     * @see applyDefaults()
     */
     ...
     * @return $this|\vgi\Style The current object (for fluent API support)
     */
    public function setName($v)
    {
        ...
        if ($this->name !== $v) {
            $this->name = $v;
            $this->modifiedColumns[StyleTableMap::COL_NAME] = true;
        }
    ...
    public function setText($v)
    {
        ...
        if ($this->text !== $v) {
            $this->text = $v;
            $this->modifiedColumns[StyleTableMap::COL_TEXT] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : StyleTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType)];
            $this->name = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : StyleTableMap::translateFieldName('Text', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\vgi\\Style'), 0, $e);
        }
    }
        ...
        $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                if ($this->archiveOnDelete) {
                    // do nothing yet. The object will be archived later when calling ChildStyleQuery::delete().
                } else {
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(StyleTableMap::COL_NAME)) {
            $modifiedColumns[':p' . $index++]  = 'name';
        }
        if ($this->isColumnModified(StyleTableMap::COL_TEXT)) {
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\vgi\Style The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(StyleTableMap::COL_NAME)) {
            $criteria->add(StyleTableMap::COL_NAME, $this->name);
        }
        if ($this->isColumnModified(StyleTableMap::COL_TEXT)) {
            $criteria->add(StyleTableMap::COL_TEXT, $this->text);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \vgi\Style (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \vgi\Style Clone of current object.
     * @throws PropelException
     */
     ...
     * @return ObjectCollection|ChildItem[] List of ChildItem objects
     */
    public function getItemsJoin_Console(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildItemQuery::create(null, $criteria);
        $query->joinWith('_Console', $joinBehavior);
     ...
     * @param ConnectionInterface $con Optional connection object
     *
     * @return     ChildStyleArchive An archive object, or null if the current object was never archived
     */
    public function getArchive(ConnectionInterface $con = null)
    {
        ...
        $archive = ChildStyleArchiveQuery::create()
            ->filterByPrimaryKey($this->getPrimaryKey())
            ->findOne($con);
     ...
     *
     * @return     ChildStyleArchive The archive object based on this object
     */
     ...
     *
     * @param      ChildStyleArchive $archive An archived object based on the same class
      *
     * @return     ChildStyle The current object (for fluent API support)
     */
