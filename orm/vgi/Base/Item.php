...
abstract class Item implements ActiveRecordInterface
{
    ...
    /**
     * @var        ChildImageData one-to-one related ChildImageData object
     */
    protected $singleImageData;
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->singleImageData !== null) {
                if (!$this->singleImageData->isDeleted() && ($this->singleImageData->isNew() || $this->singleImageData->isModified())) {
                    $affectedRows += $this->singleImageData->save($con);
                }
            }
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            ...
            if (null !== $this->singleImageData) {
                ...
                $result[$key] = $this->singleImageData->toArray($keyType, $includeLazyLoadColumns, $alreadyDumpedObjects, true);
            }
        }
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy) {
            ...
            $relObj = $this->getImageData();
            if ($relObj) {
                $copyObj->setImageData($relObj->copy($deepCopy));
            }
    ...
    /**
     * Gets a single ChildImageData object, which is related to this object by a one-to-one relationship.
     *
     * @param  ConnectionInterface $con optional connection object
     * @return ChildImageData
     * @throws PropelException
     */
    public function getImageData(ConnectionInterface $con = null)
    {

        if ($this->singleImageData === null && !$this->isNew()) {
            $this->singleImageData = ChildImageDataQuery::create()->findPk($this->getPrimaryKey(), $con);
        }
    ...
    /**
     * Sets a single ChildImageData object as related to this object by a one-to-one relationship.
     *
     * @param  ChildImageData $v ChildImageData
     * @return $this|\vgi\Item The current object (for fluent API support)
     * @throws PropelException
     */
    public function setImageData(ChildImageData $v = null)
    {
        $this->singleImageData = $v;

        // Make sure that that the passed-in ChildImageData isn't already associated with this object
        if ($v !== null && $v->get_ImageUrl(null, false) === null) {
            $v->set_ImageUrl($this);
        }
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep) {
            ...
            if ($this->singleImageData) {
                $this->singleImageData->clearAllReferences($deep);
            }
        } // if ($deep)
