 ...
 *
 * @method     ChildItemQuery leftJoinImageData($relationAlias = null) Adds a LEFT JOIN clause to the query using the ImageData relation
 * @method     ChildItemQuery rightJoinImageData($relationAlias = null) Adds a RIGHT JOIN clause to the query using the ImageData relation
 * @method     ChildItemQuery innerJoinImageData($relationAlias = null) Adds a INNER JOIN clause to the query using the ImageData relation
 *
 * @method     ChildItemQuery joinWithImageData($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the ImageData relation
 *
 * @method     ChildItemQuery leftJoinWithImageData() Adds a LEFT JOIN clause and with to the query using the ImageData relation
 * @method     ChildItemQuery rightJoinWithImageData() Adds a RIGHT JOIN clause and with to the query using the ImageData relation
 * @method     ChildItemQuery innerJoinWithImageData() Adds a INNER JOIN clause and with to the query using the ImageData relation
 *
 * @method     \vgi\ConsoleQuery|\vgi\CategoryQuery|\vgi\StateQuery|\vgi\StyleQuery|\vgi\ExtraQuery|\vgi\PriceListQuery|\vgi\ImageDataQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
...
protected $entityNotFoundExceptionClass = '\\Propel\\Runtime\\Exception\\EntityNotFoundException';
    ...
    /**
     * Filter the query by a related \vgi\ImageData object
     *
     * @param \vgi\ImageData|ObjectCollection $imageData the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return ChildItemQuery The current query, for fluid interface
     */
    ...
    {
        if ($imageData instanceof \vgi\ImageData) {
            return $this
                ->addUsingAlias(ItemTableMap::COL_IMAGE_URL, $imageData->getImageUrl(), $comparison);
        } elseif ($imageData instanceof ObjectCollection) {
            return $this
                ->useImageDataQuery()
                ->filterByPrimaryKeys($imageData->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByImageData() only accepts arguments of type \vgi\ImageData or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \vgi\ImageDataQuery A secondary query class using the current class as primary query
     */
    public function useImageDataQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinImageData($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'ImageData', '\vgi\ImageDataQuery');
    }
